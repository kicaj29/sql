...
REM Ten pakiet demonstruje dzia³ania dotycz¹ce sk³adowanych tabel zagnie¿d¿onych.
...
-- Wyœwietlenie listy studentów, którzy wypo¿yczyli okreœlon¹ ksi¹¿kê.
  ...
  BEGIN
    -- Pobranie ca³ej tabeli zagnie¿d¿onej do zmiennej PL/SQL.
    SELECT wypozyczono_dla
      INTO z_ListaStudentow
      FROM katalog_bibl
    ...
    IF z_ListaStudentow IS NOT NULL THEN
      FOR z_Indeks IN 1..z_ListaStudentow.COUNT LOOP
        z_Znaleziono := TRUE;
        
        SELECT *
          INTO z_Student
          FROM studenci
          WHERE ID = z_ListaStudentow(z_Indeks);
